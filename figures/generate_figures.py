...
def plot_fig2_ppc(pkl_paths, labels, colors=None, save_path=None):
    ...
    ax = axes[1]
    zero_obs_vals, zero_sim_vals, zero_sim_errs = [], [], []
    for model_id, pkl_path in pkl_paths.items():
        data = load_pkl(pkl_path)
        res = data['res']
        y_obs = data['data']['y']
    ...
    ax.bar(x + width/2, zero_sim_vals, width, yerr=zero_sim_errs, label='Simulated', color=[colors.get(n.split('-')[0], 'gray') for n in model_names], alpha=0.7, capsize=3)
    ...
    ax = axes[2]
    p99_obs_vals, p99_sim_vals, p99_sim_errs = [], [], []
    for model_id, pkl_path in pkl_paths.items():
        data = load_pkl(pkl_path)
        res = data['res']
    ...
    ax.bar(x + width/2, p99_sim_vals, width, yerr=p99_sim_errs, label='Simulated', color=[colors.get(n.split('-')[0], 'gray') for n in model_names], alpha=0.7, capsize=3)
